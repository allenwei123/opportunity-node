var app = angular.module('postBasic', [{
    files:[
        "root/organize/views/postBasic/_res/js/service.js"
    ]
}]);
app.controller('postBasicCtrl',function ($scope,$state) {

    if ($state.current.url == '/postBasic') {//默认加载列表
        $state.go('root.organize.views.postBasic.list[12]');
    }

}).controller('postBasicMenuCtrl',function($scope,$state,$rootScope,$location){
