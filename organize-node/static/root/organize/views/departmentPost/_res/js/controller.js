var app = angular.module('departmentPost', [{
    files:[
        "root/organize/views/departmentPost/_res/js/service.js"
    ]
}]);
app.controller('departmentPostCtrl',function ($scope,$state) {

    if ($state.current.url == '/departmentPost') {//默认加载列表
        $state.go('root.organize.views.departmentPost.list[12]');
    }

}).controller('departmentPostMenuCtrl',function($scope,$state,$rootScope,$location){
