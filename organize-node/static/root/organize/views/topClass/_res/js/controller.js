var app = angular.module('topClass', [{
    files:[
        "root/organize/views/topClass/_res/js/service.js"
    ]
}]);
app.controller('topClassCtrl',function ($scope,$state) {

    if ($state.current.url == '/topClass') {//默认加载列表
        $state.go('root.organize.views.topClass.list[12]');
    }

}).controller('topClassMenuCtrl',function($scope,$state,$rootScope,$location){
