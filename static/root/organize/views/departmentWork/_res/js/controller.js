var app = angular.module('departmentWork', [{
    files:[
        "root/organize/views/departmentWork/_res/js/service.js"
    ]
}]);
app.controller('departmentWorkCtrl',function ($scope,$state) {

    if ($state.current.url == '/departmentWork') {//默认加载列表
        $state.go('root.organize.views.departmentWork.list[12]');
    }

}).controller('departmentWorkMenuCtrl',function($scope,$state,$rootScope,$location){
