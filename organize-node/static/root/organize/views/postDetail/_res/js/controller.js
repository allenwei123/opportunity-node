var app = angular.module('postDetail', [{
    files:[
        "root/organize/views/postDetail/_res/js/service.js"
    ]
}]);
app.controller('postDetailCtrl',function ($scope,$state) {

    if ($state.current.url == '/postDetail') {//默认加载列表
        $state.go('root.organize.views.postDetail.list[12]');
    }

}).controller('postDetailMenuCtrl',function($scope,$state,$rootScope,$location){
